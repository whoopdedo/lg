...
protected:
	...
	static void _resize(void **p, uint sz, uint c) SPEC_THROW(std::bad_alloc);
...
public:
	...
	ulong size() const
		{ return m_size; }
	ref operator[](uint _n) SPEC_THROW(std::out_of_range);
	c_ref operator[](uint _n) const SPEC_THROW(std::out_of_range);
	...
	uint append(c_ref _e);
	uint insert(c_ref _e, uint _n);
	void erase(uint _n) SPEC_THROW(std::out_of_range);
};
